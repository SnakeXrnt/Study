...
namespace Week3
{
    public class MySqlCoordinatorRepository : ICoordinatorRepository, IDisposable
    {
        ...
        {
            // Drop dependent tables first if they exist (to handle schema updates)
            var dropSql = @"
...
";
            using (var dropCmd = _conn!.CreateCommand())
            {
...
CREATE TABLE assignments (
  ...
  minor_module VARCHAR(200),
  is_completed INT(1) DEFAULT 0,
  CONSTRAINT FK_assign_intern_2 FOREIGN KEY (internship_id) REFERENCES internships(id) ON DELETE CASCADE
);
";
