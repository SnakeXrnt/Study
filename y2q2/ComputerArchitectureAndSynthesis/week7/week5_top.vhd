...
begin
    ...
    -- Branch decision: 
    -- BEQ (funct3=000): Branch when Zero=1 (equal)
    -- BNE (funct3=001): Branch when Zero=0 (not equal)
    PCSrc <= (Branch AND Zero) when instr(12) = '0' else       -- BEQ
             (Branch AND (not Zero));                          -- BNE
