...
static const char *GESTURE_NAMES[] = {"COBRA", "UP", "DOWN", "LEFT", "RIGHT", "STILL"};

// --- Global/Static pointers for callbacks ---
#if defined(ARDUINO_ARCH_ESP32)
#include <NimBLEDevice.h>
static BleConnectionEnum::NotifyCallback _clientCallback       = nullptr;
static BleConnectionEnum::DiscoveryCallback _discoveryCallback = nullptr;
...
#if defined(ARDUINO_ARCH_NRF52840) || defined(ARDUINO_ARCH_NRF52) || defined(ARDUINO_ARCH_MBED)
#include <ArduinoBLE.h>
static BLEService *_pService     = nullptr;
static BLECharacteristic *_pChar = nullptr;
#endif

BleConnectionEnum::BleConnectionEnum() : _seqOut(0) {}

const char *BleConnectionEnum::getGestureName(Gesture gesture) {
    if (gesture >= 0 && gesture <= 5) return GESTURE_NAMES[gesture];
    return "UNKNOWN";
}

Gesture BleConnectionEnum::stringToGesture(const char *name) {
    for (int i = 0; i < 6; i++) {
...
int BleConnectionEnum::packGesture(uint8_t *buf, uint8_t seq, uint32_t ts, Gesture gesture) {
    int off    = 0;
    buf[off++] = seq;
...
#if defined(ARDUINO_ARCH_NRF52840) || defined(ARDUINO_ARCH_NRF52) || defined(ARDUINO_ARCH_MBED)
bool BleConnectionEnum::initServer(const char *deviceName) {
...
bool BleConnectionEnum::isConnected() {
    return BLE.central().connected();
}

void BleConnectionEnum::sendCommand(Gesture gesture) {
    _seqOut++;
    uint8_t pkt[20];
...
void BleConnectionEnum::sendCommand(const char *name) {
    sendCommand(stringToGesture(name));
}

void BleConnectionEnum::updateServer() {
    BLE.poll();
    ...
    if (!wasConnected && connected) { Serial.println("Central connected!"); }
    wasConnected = connected;
}

void BleConnectionEnum::disconnect() {
...
class ScanCallbacks : public NimBLEAdvertisedDeviceCallbacks {
    void onResult(NimBLEAdvertisedDevice *advertisedDevice) override {
        ...
        if (isTarget && !_connected && !_doConnect && !_connecting) {
            Serial.printf(
                "Target detected! Name: %s, Addr: %s, RSSI: %d\n",
                advertisedDevice->getName().c_str(),
                advertisedDevice->getAddress().toString().c_str(),
                advertisedDevice->getRSSI()
            );
...
static void internalNotifyCallback(
...
bool BleConnectionEnum::initClient(NotifyCallback callback, DiscoveryCallback discovery) {
...
bool BleConnectionEnum::isScanning() {
    return _scanning;
}
bool BleConnectionEnum::isConnecting() {
    return _connecting;
}

void BleConnectionEnum::connect() {
    _doConnect = true;
}
...
void BleConnectionEnum::updateClient() {
    if (_doConnect) {
