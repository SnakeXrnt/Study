 ...
 * This sketch will print the IP address of your WiFi module to the Serial
 * Monitor You can use that IP address with a web browser to turn on and off the
 * LED on pin 9.
 *
...
char ssid[] = "Ziggo1395598"; // your network SSID (name)
char pass[] = "d5nNgGc2byjv"; // your network password
...
void loop() {
  ...
  if (client) {
    ...
    Serial.println(
        "New client connected"); // print a message out the serial port
    String currentLine =
      ...
      if (c == '\n') { // if the byte is a newline character
        ...
        if (currentLine.length() == 0) {
          ...
          client.print("Click <a href=\"/H\">here</a> turn the LED on "
                       "LED_BUILTIN on<br>\n");
          client.print("Click <a href=\"/L\">here</a> turn the LED on "
                       "LED_BUILTIN off<br>");
...
/*
 * Read data in the WiFi device. Reading byte by byte in very slow. This
 * function allows to read READ_BUFFER byte in the device and returns byte by
 * byte the result.
 */
int dataRead(char *c) {
