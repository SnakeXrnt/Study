...
BleConnection ble;
BleKeyboard bleKeyboard("TTGO Gesture HID", "Saxion", 100);
...
void onData(uint8_t seq, uint32_t ts, Gesture g) {
    ...
    // HID Media Control Mapping
    if (bleKeyboard.isConnected()) {
        switch (g) {
            case GESTURE_LEFT:
                Serial.println(">>> [HID] Previous Track");
                bleKeyboard.write(KEY_MEDIA_PREVIOUS_TRACK);
                break;
            case GESTURE_RIGHT:
                Serial.println(">>> [HID] Next Track");
                bleKeyboard.write(KEY_MEDIA_NEXT_TRACK);
                break;
            case GESTURE_UP:
                Serial.println(">>> [HID] Volume Up");
                bleKeyboard.write(KEY_MEDIA_VOLUME_UP);
                break;
            case GESTURE_DOWN:
                Serial.println(">>> [HID] Volume Down");
        ...
        Serial.println(">>> [HID] Keyboard not connected, skipping command.");
    }
}
...
void setup() {
    ...
    // 1. Start HID Keyboard FIRST to set the device name correctly
    bleKeyboard.begin();
    Serial.println(">>> HID KEYBOARD STARTED.");

    // 2. Initialize the BLE Client (it will see that NimBLE is already init'd)
