...
static const char* GESTURE_NAMES[] = {"COBRA", "UP", "DOWN", "LEFT", "RIGHT", "NONE"};
...
void BleConnection::updateClient() {
    if (_doConnect) {
        ...
        _scanning = false;
        delay(100); // Shortest possible delay for stack stability
        ...
        if (_targetAddress.toString() == "00:00:00:00:00:00") {
            ...
            // NimBLE connect is blocking, but we've already drawn the UI.
            if (_pClient->connect(_targetAddress)) {
                ...
                Serial.println(">>> [BLE] Service/Char discovery failed.");
            } else {
